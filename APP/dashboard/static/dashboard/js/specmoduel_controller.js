...
class controller extends model {
						...
						const html = `
						  <div class="card course_outline_card mb-2">
						    <div class="card-header course_outline_cell" id="heading-${module}" style=''>
								...
								<button class="btn text-left text-primary" type="button" onclick="controller.C_restore_module('${url}', '${csrf_token}', '${spec_id}','${module}')">
									Restore
								</button>
						    </div>
						  </div>
	...
	C_restore_module(url, csrf_token, spec_id, module) {
		//send command
		// Get a reference to the modal element
						...
						<div class="accordion mb-1" id="Module_${module}">
						  <div class="card course_outline_card">
						    <div class="card-header course_outline_cell" id="heading-${module}" style=''>
								...
								<button class="btn text-left text-danger" type="button" onclick="controller.C_remove_module('${url}', '${csrf_token}', '${spec_id}','${module}')">
									Remove
								</button>
						    </div>
	...
	C_remove_chapter(url, csrf_token, spec_id, module, chapter) {
		//send command
		// Get a reference to the modal element
					...
					if (json.error == 0){
						...
						const html = `
							<div class='' style='cursor:pointer;' id='Remove_chapter_${module}_${chapter}'>
									<span class='font-weight-bold ml-2' id='course_outline_span'><i class="bi bi-arrow-bar-right course_outline_icon"></i> <a href="{%url 'content:noteedit' context.spec.id module chapter%}">Chapter - ${chapter}</a></span>
									<button class="btn text-left text-primary" type="button">
										Restore
									</button>
							</div>
						`
						const removed_modules = document.getElementById('remove_chapters_div');
						removed_modules.innerHTML += html
						chapter_card.remove();
					}else{
						s.innerHTML = 'error :('
